...
/**
 * Post-fusion VGN chroma filtering for MGC Spatial RGB.
 *
 * MGC Spatial RGB tiles are assembled into one full-size 2D texture before chroma filtering. All
 * later shaders use direct global coordinates over that contiguous surface, so local kernels cross
 * tile seams naturally and every IIR invocation scans one physical output row or column without
...
internal class GlesMgcSpatialRgbChromaPostprocessor(
    ...
    fun process(
        ...
        val timing = if (cpuBufferPopulated) {
            ...
            // The callback inserted a later command-stream fence whose ownership leaves this
            // component with the exported texture. Its checkpoint supersedes the local fences.
            passWindow.clearAfterCheckpoint()
            ReadbackTiming(
                elapsedMs = 0L,
                gpuQueueWaitMs = 0L,
...
internal data class MgcSpatialRgbChromaIirCoefficients(
    ...
    companion object {
        ...
        /** Keeps the same RAW-space cutoff when the MGC Spatial RGB sample grid is upscaled. */
