...
internal object DngPhotonLocalToneMapper {
    ...
    internal const val SOURCE_EPSILON = 1e-6f
    ...
    fun generate(
        ...
        val targetBeforeBaselineExposure = FloatArray(source.size) { index ->
            localToneMap.target[index] / photonPlan.exposureGain
        }
        val gainGrid = fitScalarGainBilateralGrid(
            source = source,
            fitInput = source,
            target = targetBeforeBaselineExposure,
            width = sampleWidth,
            height = sampleHeight,
            gridWidth = plan.grid.mapPointsH,
            gridHeight = plan.grid.mapPointsV,
    ...
    internal fun localLaplacianToneMap(
        ...
        source.forEachIndexed { index, value ->
            // Retaining the epsilon through postprocessing gives RAW black a defined finite
            // intensity. MATLAB's HDR inputs are strictly positive, while normalized RAW can be 0.
            val exposedValue =
                exposureGain.toDouble() * value.toDouble() + SOURCE_EPSILON.toDouble()
            exposedInput[index] = exposedValue.toFloat()
            val logValue = ln(exposedValue).toFloat()
        ...
        val targetLogDynamicRange = ln(parameters.targetDynamicRange.toDouble()).toFloat()
        val inputLogDynamicRange = ln(
            (inputUpper / inputLower).toDouble()
        ).toFloat()
        val edgeSlope = if (!inputLogDynamicRange.isFinite() ||
            inputLogDynamicRange <= CURVE_EPS
        ) {
            1f
        } else {
        ...
        if (!logRange.isFinite() || logRange <= CURVE_EPS) {
            return PhotonLocalToneMapResult(
                target = FloatArray(source.size) { index ->
                    (exposedInput[index] - SOURCE_EPSILON).coerceIn(0f, 1f)
                },
            )
        }
        ...
        return PhotonLocalToneMapResult(
            target = FloatArray(source.size) { index ->
                ...
                (exp(filteredLog.toDouble()) - SOURCE_EPSILON).toFloat().also { value ->
                    require(value.isFinite()) {
                        "Local Laplacian produced invalid radiance at $index: $value"
                    }
                }.coerceIn(0f, 1f)
            },
        )
    ...
    private fun percentile(sorted: FloatArray, quantile: Float): Float {
        ...
        val position = quantile.coerceIn(0f, 1f) * (sorted.size - 1)
...
internal data class PhotonLocalToneMapResult(
    ...
    init {
        require(target.all { it.isFinite() && it in 0f..1f })
    }
}
