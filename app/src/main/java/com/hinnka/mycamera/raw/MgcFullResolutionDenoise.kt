 ...
 *
 * This class owns luma/chroma protobuf tuning selection and noise preparation.
 * Multi-frame correlation and coefficient scaling must come from the Spatial
 * merge's actual quantized masks, never from frame count.
 */
internal object MgcFullResolutionDenoise {
    ...
    /**
     * Runs MGC RunFullResolutionDenoise in place on camera-domain RGBA16F.
     * Camera RGB pixels have already received LSC in the demosaic stage; the
     * LSC map is consumed only by MGC's strength-map kernel.
     */
    fun denoise(
        rgba16f: ByteBuffer,
        width: Int,
        ...
        if (useSpatialModel && metadata.frameCount > 1 &&
            ...
            PLog.w(
                TAG,
                "MGC denoise skipped: fused frameCount=${metadata.frameCount} " +
                    "has no complete propagated merge noise model",
            )
            return false
        }
        if (correlation != null &&
            ...
            PLog.e(TAG, "MGC denoise rejected malformed correlation spectrum")
            return false
        }
        val noiseScale = if (useSpatialModel) {
            metadata.mgcDenoiseNoiseScale ?: 1f
        } else {
            1f
        }
        if (!noiseScale.isFinite() || noiseScale <= 0f) {
        ...
        PLog.d(
            ...
            "MGC static denoise complete: size=${width}x$height " +
                "origin=($globalOriginX,$globalOriginY) gain=$tuningGain pass=$pass " +
    ...
    private fun bayerNoiseToRgb(
        channels: FloatArray,
        scale: Float,
    ): FloatArray {
        val red = channels.getOrElse(0) { 0f }
        val green = 0.5f * (
            channels.getOrElse(1) { 0f } +
                channels.getOrElse(2) { channels.getOrElse(1) { 0f } }
            )
