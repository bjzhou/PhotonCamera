...
/**
 * Plans DNG ProfileGainTableMap2 metadata for capture local tone mapping.
 *
 * MGC AE capture uses the synthetic-exposure LTM plan carried by [MgcLtmCapturePlan]. The legacy
 * Local Laplacian parameters remain available only for non-AE/manual Photon-HDR requests that do
 * not have an MGC plan. Both paths use the same bilateral-grid fit solely to encode their scalar
 * linear target into DNG's ProfileGainTableMap representation.
 */
internal object DngPhotonProfileGainTableGenerator {
    ...
    /** Paris et al. intensity used by the current full-resolution Local Laplacian pipeline. */
