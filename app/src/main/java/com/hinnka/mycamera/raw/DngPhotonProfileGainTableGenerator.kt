...
/**
 * Plans Photon HDR metadata for the sampled Local Laplacian + bilateral-grid pipeline.
 *
 ...
 * highlights therefore do not determine exposure. Photon uses max RGB as both the Local Laplacian
 * intensity and the DNG table coordinate, so the gain follows and bounds the brightest channel
 * without changing RGB ratios. The MATLAB example's final display gamma is omitted because output
