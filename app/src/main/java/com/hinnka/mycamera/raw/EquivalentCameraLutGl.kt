...
internal object EquivalentCameraLutShader {
    val UNIFORMS = """
        uniform bool uEquivalentCameraEnabled;
    ...
    val FUNCTIONS = """
            ...
            vec3 equivalentCameraRgb(vec3 camera) {
                if (!uEquivalentCameraEnabled) return camera;
...
internal class EquivalentCameraLutGl {
    ...
    fun bind(program: Int, luts: EquivalentCameraLuts?, firstWeight: Float) {
        GLES30.glUniform1i(GLES30.glGetUniformLocation(program, "uEquivalentCameraEnabled"), if (luts != null) 1 else 0)
        GLES30.glUniform1f(GLES30.glGetUniformLocation(program, "uEquivalentCameraFirstWeight"), firstWeight)
        // Even a disabled sampler needs a complete texture on a distinct unit.
        // These 1x1 placeholders are never sampled; bypass does no LUT baking or disk I/O.
        val key = luts?.key ?: "direct-camera-rgb"
        val size = luts?.size ?: 1
        ...
        try {
            for (index in units.indices) {
                ...
                if (uploadedCalibrationKey == key) continue
                val values = if (luts == null) floatArrayOf(0f, 0f, 0f)
                    else if (index == 0) luts.low else luts.high
                ...
                GLES30.glTexImage3D(GLES30.GL_TEXTURE_3D, 0, GLES30.GL_RGB32F, size, size, size,
