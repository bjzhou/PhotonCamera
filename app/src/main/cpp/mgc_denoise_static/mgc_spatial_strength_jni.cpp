...
Java_com_hinnka_mycamera_processor_MgcSpatialStrengthMapGenerator_nativeCompute(
    ...
    if (fused_f16_buffer == nullptr || alignment_buffer == nullptr ||
    ...
    auto* fused_f16 = static_cast<uint16_t*>(
        env->GetDirectBufferAddress(fused_f16_buffer));
    ...
    if (fused_f16 == nullptr || alignment == nullptr || rejection == nullptr) {
        return -1;
    }
    const int64_t fused_samples =
        static_cast<int64_t>(width) * height * (layout == 0 ? 1 : 4);
    ...
    const int64_t output_values =
        static_cast<int64_t>(rejection_width) * rejection_height;
    if (env->GetDirectBufferCapacity(fused_f16_buffer) <
            fused_samples * static_cast<int64_t>(sizeof(uint16_t)) ||
    ...
    try {
        ...
        const int result = photon::mgc_denoise::ComputeSpatialStrengthMap(
            layout == 0
                ...
                : photon::mgc_denoise::SpatialStrengthInputLayout::Rgba,
            fused_f16,
            width,
            height,
            cfa_pattern,
        ...
        __android_log_print(
            ...
            "q8=%u..%u/%.3f noise0=[%.6g,%.6g,%.6g] "
            "noise1=[%.6g,%.6g,%.6g] diag0=[%.6g,%.6g,%.6g] "
            "diag1=[%.6g,%.6g,%.6g]",
            layout == 0 ? "BAYER" : "RGB",
            rejection_width,
            rejection_height,
