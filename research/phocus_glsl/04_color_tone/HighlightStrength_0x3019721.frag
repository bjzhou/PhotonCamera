 ...
 * SHA-256: 7e01c980ca698783302664399c9fbbf037d161e4df32398671c1faab48b60701
 * ELF offsets: 0x3019721
 * Symbols: HighlightStrengthShader
 */
...
vec3 HighlightL(vec3 input, vec2 X, vec3 P, vec2 L) {
    vec3 resP = input * input * P.x + input * P.y + P.z;
    vec3 resL = input * L.x + L.y;
    return mix(mix(resL, resP, step(X.y, input)), input, step(X.x, input));
}

vec3 HighlightP(vec3 input, vec2 X, vec3 P, vec3 Ph) {
    vec3 resP = input * input * P.x + input * P.y + P.z;
    vec3 resPh = input * input * Ph.x + input * Ph.y + Ph.z;
    return mix(mix(resPh, resP, step(X.y, input)), input, step(X.x, input));
}
...
//    rgbIn = pow(rgbIn, vec3(kGammaHasselbladRgb));
...
//    rgbHStrength = pow(rgbHStrength, vec3(1.0 / kGammaHasselbladRgb));
...
void main() {
    ...
    FragColor = vec4(result, pixelColor.a);
}
