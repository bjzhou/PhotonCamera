.class public final Lrcp;
.super Ljava/lang/Object;
.source "PG"

# interfaces
.implements Lrbz;


# instance fields
.field private final synthetic a:I


# direct methods
.method public constructor <init>(I)V
    .locals 0

    goto/32 :goto_0

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_0
    iput p1, p0, Lrcp;->a:I

    nop

    goto/32 :goto_1

    nop

    nop

    nop

    nop

    nop

    :goto_1
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    goto/32 :goto_2

    nop

    nop

    nop

    nop

    :goto_2
    return-void

    nop

    nop
.end method


# virtual methods
.method public final a(Lrcs;Landroid/view/View;)V
    .locals 5

    goto/32 :goto_2a

    nop

    nop

    nop

    :goto_0
    invoke-interface {p1, v4, p2}, Lrcs;->c(Ljava/lang/CharSequence;Z)V

    goto/32 :goto_1

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_1
    sget-object p2, Landroid/view/ViewOutlineProvider;->BACKGROUND:Landroid/view/ViewOutlineProvider;

    nop

    goto/32 :goto_30

    nop

    nop

    nop

    nop

    :goto_2
    goto :goto_6

    nop

    :goto_3
    goto/32 :goto_1a

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_4
    invoke-virtual {p0}, Landroid/graphics/Rect;->toShortString()Ljava/lang/String;

    move-result-object p0

    nop

    nop

    nop

    nop

    nop

    goto/32 :goto_23

    nop

    nop

    :goto_5
    const-string p0, "EMPTY"

    nop

    nop

    :goto_6
    goto/32 :goto_32

    nop

    nop

    :goto_7
    invoke-virtual {v0}, Landroid/graphics/Outline;->getRadius()F

    move-result p2

    nop

    nop

    nop

    goto/32 :goto_37

    nop

    nop

    nop

    nop

    :goto_8
    goto/16 :goto_45

    nop

    nop

    nop

    :goto_9
    goto/32 :goto_44

    nop

    nop

    nop

    nop

    nop

    :goto_a
    invoke-virtual {v0}, Landroid/graphics/Outline;->getAlpha()F

    move-result p0

    nop

    nop

    goto/32 :goto_35

    nop

    nop

    nop

    :goto_b
    iget-object v0, p0, Lbo;->R:Landroid/view/View;

    nop

    nop

    nop

    nop

    nop

    goto/32 :goto_65

    nop

    nop

    nop

    :goto_c
    invoke-virtual {p2}, Landroid/view/View;->getOutlineProvider()Landroid/view/ViewOutlineProvider;

    move-result-object p0

    nop

    goto/32 :goto_3f

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_d
    const-string p0, "PATH"

    nop

    nop

    nop

    nop

    nop

    goto/32 :goto_2e

    nop

    nop

    nop

    nop

    :goto_e
    const/4 v2, 0x1

    nop

    nop

    nop

    nop

    nop

    goto/32 :goto_2c

    nop

    nop

    nop

    nop

    nop

    :goto_f
    if-nez p0, :cond_0

    nop

    goto/32 :goto_53

    nop

    nop

    nop

    nop

    nop

    :cond_0
    goto/32 :goto_20

    nop

    nop

    :goto_10
    if-ne v1, p0, :cond_1

    nop

    nop

    goto/32 :goto_3

    nop

    :cond_1
    goto/32 :goto_3b

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_11
    invoke-direct {p0}, Landroid/graphics/Rect;-><init>()V

    goto/32 :goto_2b

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_12
    iget-object p0, p0, Lbo;->I:Ljava/lang/String;

    nop

    nop

    nop

    nop

    nop

    goto/32 :goto_f

    nop

    nop

    nop

    nop

    :goto_13
    if-nez p0, :cond_2

    nop

    nop

    nop

    goto/32 :goto_53

    nop

    :cond_2
    goto/32 :goto_b

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_14
    return-void

    nop

    nop

    nop

    nop

    nop

    :goto_15
    goto/32 :goto_28

    nop

    nop

    nop

    nop

    nop

    :goto_16
    invoke-interface {p1, p2, p0}, Lrcs;->b(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)V

    goto/32 :goto_61

    nop

    nop

    nop

    nop

    nop

    :goto_17
    goto/16 :goto_6

    nop

    nop

    nop

    :goto_18
    goto/32 :goto_5

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_19
    const-string p0, "BOUNDS"

    nop

    nop

    nop

    goto/32 :goto_50

    nop

    nop

    nop

    nop

    nop

    :goto_1a
    const-string p0, "NOT_EMPTY"

    nop

    goto/32 :goto_17

    nop

    nop

    nop

    nop

    nop

    :goto_1b
    if-nez v1, :cond_3

    nop

    goto/32 :goto_34

    nop

    nop

    nop

    :cond_3
    goto/32 :goto_1e

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_1c
    invoke-static {p0}, Ljava/lang/String;->valueOf(Ljava/lang/Object;)Ljava/lang/String;

    move-result-object p0

    nop

    nop

    :goto_1d
    goto/32 :goto_5d

    nop

    nop

    :goto_1e
    move v1, v2

    nop

    nop

    nop

    nop

    nop

    goto/32 :goto_33

    nop

    nop

    nop

    nop

    nop

    :goto_1f
    invoke-virtual {p2}, Landroid/view/View;->getClipToOutline()Z

    move-result p2

    nop

    nop

    nop

    nop

    nop

    goto/32 :goto_4b

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_20
    const-string p2, "fragment_tag"

    nop

    nop

    nop

    nop

    nop

    goto/32 :goto_52

    nop

    nop

    :goto_21
    cmpl-float v1, v1, v4

    nop

    nop

    nop

    goto/32 :goto_40

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_22
    const v1, 0x1b

    nop

    nop

    nop

    nop

    goto/32 :goto_66

    nop

    nop

    :goto_23
    const-string p2, "outline_rect"

    nop

    nop

    nop

    goto/32 :goto_46

    nop

    nop

    nop

    nop

    nop

    :goto_24
    iget p0, p0, Lrcp;->a:I

    nop

    nop

    nop

    nop

    goto/32 :goto_41

    nop

    nop

    nop

    :goto_25
    sget-object p2, Landroid/view/ViewOutlineProvider;->PADDED_BOUNDS:Landroid/view/ViewOutlineProvider;

    nop

    nop

    nop

    nop

    goto/32 :goto_5c

    nop

    nop

    nop

    nop

    nop

    :goto_26
    goto :goto_1d

    nop

    nop

    nop

    nop

    nop

    :goto_27
    goto/32 :goto_1c

    nop

    nop

    :goto_28
    goto/32 :goto_64

    nop

    nop

    :goto_29
    const-string v0, "fragment"

    nop

    goto/32 :goto_38

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_2a
    const v0, 0xb

    nop

    nop

    nop

    nop

    nop

    goto/32 :goto_22

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_2b
    invoke-virtual {v0, p0}, Landroid/graphics/Outline;->getRect(Landroid/graphics/Rect;)Z

    goto/32 :goto_7

    nop

    nop

    nop

    :goto_2c
    const/4 v3, 0x3

    nop

    nop

    nop

    nop

    goto/32 :goto_1b

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_2d
    const-string p0, "BACKGROUND"

    nop

    nop

    goto/32 :goto_59

    nop

    nop

    :goto_2e
    goto/16 :goto_6

    nop

    nop

    nop

    nop

    nop

    :goto_2f
    goto/32 :goto_4f

    nop

    nop

    :goto_30
    if-eq p0, p2, :cond_4

    nop

    nop

    goto/32 :goto_5a

    nop

    :cond_4
    goto/32 :goto_2d

    nop

    nop

    :goto_31
    sget-object p2, Landroid/view/ViewOutlineProvider;->BOUNDS:Landroid/view/ViewOutlineProvider;

    nop

    goto/32 :goto_4e

    nop

    nop

    :goto_32
    const-string p2, "outline_mode"

    nop

    goto/32 :goto_5e

    nop

    nop

    :goto_33
    goto/16 :goto_45

    nop

    nop

    nop

    nop

    :goto_34
    goto/32 :goto_4c

    nop

    nop

    nop

    nop

    :goto_35
    const-string p2, "outline_alpha"

    nop

    nop

    goto/32 :goto_43

    nop

    nop

    :goto_36
    if-eq v1, v3, :cond_5

    nop

    nop

    nop

    goto/32 :goto_47

    nop

    nop

    nop

    :cond_5
    goto/32 :goto_42

    nop

    nop

    nop

    nop

    nop

    :goto_37
    const-string v0, "outline_radius"

    nop

    goto/32 :goto_39

    nop

    nop

    nop

    nop

    :goto_38
    invoke-virtual {p2}, Ljava/lang/Class;->getName()Ljava/lang/String;

    move-result-object p2

    nop

    goto/32 :goto_55

    nop

    nop

    :goto_39
    invoke-interface {p1, v0, p2}, Lrcs;->d(Ljava/lang/CharSequence;F)V

    goto/32 :goto_4

    nop

    nop

    :goto_3a
    const/4 v4, 0x0

    nop

    nop

    nop

    nop

    goto/32 :goto_21

    nop

    nop

    nop

    nop

    :goto_3b
    if-ne v1, v3, :cond_6

    nop

    nop

    nop

    nop

    goto/32 :goto_2f

    nop

    nop

    nop

    nop

    :cond_6
    goto/32 :goto_d

    nop

    nop

    :goto_3c
    const/4 p0, 0x0

    nop

    nop

    :goto_3d
    goto/32 :goto_13

    nop

    nop

    nop

    nop

    nop

    :goto_3e
    invoke-virtual {p0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    move-result-object p2

    nop

    nop

    nop

    goto/32 :goto_29

    nop

    nop

    nop

    :goto_3f
    new-instance v0, Landroid/graphics/Outline;

    nop

    nop

    nop

    nop

    goto/32 :goto_57

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_40
    if-gez v1, :cond_7

    nop

    nop

    nop

    goto/32 :goto_9

    nop

    nop

    nop

    nop

    nop

    :cond_7
    goto/32 :goto_4a

    nop

    nop

    nop

    nop

    nop

    :goto_41
    if-nez p0, :cond_8

    nop

    goto/32 :goto_60

    nop

    nop

    nop

    :cond_8
    :try_start_0
    invoke-static {p2}, Lcj;->e(Landroid/view/View;)Lbo;

    move-result-object p0

    nop

    nop

    nop

    if-eqz p0, :cond_9

    nop

    nop

    nop

    goto :goto_3d

    nop

    nop

    nop

    nop

    :cond_9
    new-instance p0, Ljava/lang/IllegalStateException;

    nop

    nop

    nop

    nop

    nop

    const-string v0, "View "

    nop

    nop

    nop

    const-string v1, " does not have a Fragment set"

    nop

    nop

    nop

    nop

    invoke-static {p2, v0, v1}, La;->bn(Ljava/lang/Object;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v0

    nop

    nop

    nop

    nop

    invoke-direct {p0, v0}, Ljava/lang/IllegalStateException;-><init>(Ljava/lang/String;)V

    throw p0

    nop

    nop

    nop

    nop

    nop
    :try_end_0
    .catch Ljava/lang/IllegalStateException; {:try_start_0 .. :try_end_0} :catch_0

    :catch_0
    goto/32 :goto_3c

    nop

    nop

    nop

    nop

    :goto_42
    new-instance p0, Landroid/graphics/Rect;

    nop

    nop

    goto/32 :goto_11

    nop

    nop

    :goto_43
    invoke-interface {p1, p2, p0}, Lrcs;->d(Ljava/lang/CharSequence;F)V

    goto/32 :goto_36

    nop

    nop

    nop

    :goto_44
    const/4 v1, 0x4

    nop

    nop

    nop

    nop

    :goto_45
    goto/32 :goto_1f

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_46
    invoke-interface {p1, p2, p0}, Lrcs;->b(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)V

    :goto_47
    goto/32 :goto_14

    nop

    nop

    nop

    :goto_48
    invoke-virtual {p0, p2, v0}, Landroid/view/ViewOutlineProvider;->getOutline(Landroid/view/View;Landroid/graphics/Outline;)V

    :goto_49
    goto/32 :goto_56

    nop

    nop

    nop

    nop

    :goto_4a
    move v1, v3

    nop

    nop

    goto/32 :goto_8

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_4b
    const-string v4, "clipToOutline"

    nop

    nop

    nop

    nop

    goto/32 :goto_0

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_4c
    invoke-virtual {v0}, Landroid/graphics/Outline;->getRadius()F

    move-result v1

    nop

    nop

    nop

    nop

    nop

    goto/32 :goto_3a

    nop

    nop

    nop

    nop

    nop

    :goto_4d
    const/4 p0, 0x2

    nop

    nop

    goto/32 :goto_10

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_4e
    if-eq p0, p2, :cond_a

    nop

    nop

    nop

    goto/32 :goto_51

    nop

    nop

    nop

    nop

    nop

    :cond_a
    goto/32 :goto_19

    nop

    nop

    nop

    nop

    nop

    :goto_4f
    const-string p0, "ROUNDED_RECT"

    nop

    nop

    nop

    nop

    goto/32 :goto_2

    nop

    nop

    nop

    :goto_50
    goto/16 :goto_1d

    nop

    nop

    :goto_51
    goto/32 :goto_25

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_52
    invoke-interface {p1, p2, p0}, Lrcs;->b(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)V

    :goto_53
    goto/32 :goto_5f

    nop

    nop

    nop

    nop

    nop

    nop

    :goto_54
    if-nez p0, :cond_b

    nop

    goto/32 :goto_49

    nop

    nop

    nop

    nop

    nop

    :cond_b
    goto/32 :goto_48

    nop

    nop

    nop

    :goto_55
    invoke-interface {p1, v0, p2}, Lrcs;->b(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)V

    goto/32 :goto_12

    nop

    nop

    nop

    nop

    :goto_56
    invoke-virtual {v0}, Landroid/graphics/Outline;->isEmpty()Z

    move-result v1

    nop

    goto/32 :goto_e

    nop

    nop

    :goto_57
    invoke-direct {v0}, Landroid/graphics/Outline;-><init>()V

    goto/32 :goto_54

    nop

    nop

    :goto_58
    const-string p0, "PADDED_BOUNDS"

    nop

    goto/32 :goto_26

    nop

    nop

    nop

    :goto_59
    goto/16 :goto_1d

    nop

    nop

    nop

    nop

    :goto_5a
    goto/32 :goto_31

    nop

    nop

    nop

    nop

    nop

    :goto_5b
    if-lez v0, :cond_c

    nop

    goto/32 :goto_64

    nop

    :cond_c
    goto/32 :goto_63

    nop

    :goto_5c
    if-eq p0, p2, :cond_d

    nop

    nop

    goto/32 :goto_27

    nop

    nop

    nop

    nop

    :cond_d
    goto/32 :goto_58

    nop

    nop

    nop

    :goto_5d
    const-string p2, "outlineProvider"

    nop

    nop

    nop

    nop

    goto/32 :goto_16

    nop

    nop

    :goto_5e
    invoke-interface {p1, p2, p0}, Lrcs;->b(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)V

    goto/32 :goto_a

    nop

    nop

    :goto_5f
    return-void

    nop

    nop

    nop

    nop

    :goto_60
    goto/32 :goto_c

    nop

    nop

    nop

    :goto_61
    if-ne v1, v2, :cond_e

    nop

    goto/32 :goto_18

    nop

    nop

    nop

    :cond_e
    goto/32 :goto_4d

    nop

    nop

    nop

    :goto_62
    rem-int v0, v0, v1

    nop

    nop

    nop

    nop

    nop

    goto/32 :goto_5b

    nop

    nop

    :goto_63
    goto/32 :goto_15

    nop

    nop

    nop

    nop

    nop

    :goto_64
    goto/32 :goto_24

    nop

    nop

    nop

    nop

    :goto_65
    if-eq v0, p2, :cond_f

    nop

    nop

    nop

    goto/32 :goto_53

    nop

    nop

    nop

    :cond_f
    goto/32 :goto_3e

    nop

    nop

    nop

    nop

    nop

    :goto_66
    add-int v0, v0, v1

    nop

    goto/32 :goto_62

    nop

    nop

    nop
.end method
