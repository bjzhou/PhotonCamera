...
只保留 Photon 当前真正消费的融合、空域降噪和锐化领域。HDR、高光、gamma、色彩和除雾不进入拟合。

| lens | 噪声相关性 | luma 五层 | 细节重建五层 | 离群抑制五层 | chroma 五层 | 频率响应 offset | 锐化 low/medium |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 0 | 0.500 | 0.312500 | 0.312500 | 0.812500 | 0.125 | 6.5 | 0.343750 |
| 1 | 0.875 | 0.171875 | 0.312500 | 0.125000 | 0.125 | 14.5 | 0.359375 |
| 2 | 0.875 | 0.234375 | 0.312500 | 0.812500 | 0.875 | 20.0 | 0.375000 |
| 3 | 0.500 | 0.406250 | 0.312500 | 0.812500 | 0.500 | 3.5 | 0.390625 |

表中的“五层”表示五个 pyramid level 使用同一数值。高频锐化样本均为 `0`。融合 gradient threshold
没有有效的逐镜头样本，因此保持 SNR 自适应；参考信号保持 Photon 自己的 `0.18` fallback。
...
| 锐化 low/medium | 0.367187500 | — | 0.087 | 中位数 |
...
`0.3125 × 5`、锐化 `[0.3671875, 0.3671875, 0]`，以及上面的完整 Sabre 节点。
