...
单帧与多帧拍摄共用 `copyStillFocusSettingsFromPreview`，继承预览请求中的 AF 模式、
手动焦距及 AF/AE 区域。普通帧、短曝光帧、长曝光帧和 HDR 包围帧均使用这条路径。
拍摄请求沿用 IDLE 触发状态，不因多帧拍摄额外触发 AF、切换 AUTO/AF_OFF，
也不把预览 CaptureResult 的镜头距离回写为固定焦距。

多帧专用的对焦等待、超时回退、焦点快照及对应策略测试已删除。
点按对焦、手动对焦和单次闪光预闪流程保留；单次闪光仍维护自己的 AF 状态，
在拍摄结束后取消其触发并恢复预览。多帧的曝光与白平衡一致性设置不再覆盖对焦。

这项调整用于消除单帧与多帧的对焦控制差异，尚未在反馈模糊的 S23 Ultra 上验证。
连续 AF 沿用预览行为，因此不再保证整个多帧序列固定于同一焦平面。

## 历史记录：已移除的多帧 AF 等待策略

以下为 2026-09-06 的排查与当时验证记录，不代表当前实现。
